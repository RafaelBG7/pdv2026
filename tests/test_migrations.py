...
class VersionedMigrationTestCase(unittest.TestCase):
    ...
    def test_empty_central_and_tenant_reach_independent_heads(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(upgrade_database(central, 'central').current_revision, 'central_0005')
            self.assertEqual(upgrade_database(tenant, 'tenant').current_revision, 'tenant_0006')
    ...
    def test_payable_decimal_migration_preserves_and_quantizes_legacy_amount(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(upgrade_database(engine, 'tenant').current_revision, 'tenant_0006')
    ...
    def test_upgrade_is_idempotent_and_preserves_existing_data(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(database_revision(engine), 'central_0005')
            engine.dispose()
    ...
    def test_database_one_revision_behind_is_detected_and_upgraded(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(upgrade_database(engine, 'central').current_revision, 'central_0005')
            engine.dispose()
